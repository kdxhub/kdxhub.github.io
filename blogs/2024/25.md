...
创建：2024-07-20 | 最后更新：2024-12-04</small><br>
...
| 描述 | 命令 |
|-|--|
| 查看手机上的用户 | `pm list users`或`dumpsys account` |
...
| rm命令：删除文件。多个文件使用空格分隔<br>⚠请谨慎操作，误删贵重数据概不负责。root机请勿删除根目录文件，有变砖风险<br>选项<br>-r：递归处理，删除目录及其子文件 **不要在根目录下执行递归操作！！**<br>-f：允许删除空气。root权限可以强制删除仅读文件<br>-v：输出执行信息 | `rm -r <文件路径>` |
...
| 开启性能模式<br>如要关闭，请执行下面的关闭规则，或是手动 关 开 关 一遍性能模式，以免部分设置键没关闭导致电量浪费| |
| *关闭均衡模式的调度*<br> | `settings put system POWER_BALANCED_MODE_OPEN 0` |
| *启动性能模式：优先分配资源给当前应用* | `settings put system POWER_PERFORMANCE_MODE_OPEN 1` |
| *启动预关闭模式：判断性能模式是否忘关？*<br> | `settings put system POWER_SAVE_PRE_HIDE_MODE performance` |
| *启动速度模式：解锁性能上限*<br> | `settings put system speed_mode 1` |
...
| 开启省电模式<br>非官方途径开启省电模式，诸如账户同步，锁屏清理等省电调度之外的功能将不会有变动 | `settings put system POWER_SAVE_MODE_OPEN 1` |
| 疑似用于增强省电效果 **「miui用？」**  | `settings put system POWER_SAVE_PRE_HIDE_MODE ultimate` |
| 关闭省电模式 | `settings put system POWER_SAVE_MODE_OPEN 0`<br>`settings put system POWER_SAVE_PRE_HIDE_MODE enhance` |
...
| 进入演示模式 | `settings put system sysui_tuner_demo_on 1`<br>`am broadcast -a com.android.systemui.demo -e command enter` |
| 退出演示模式 | `settings put system sysui_tuner_demo_on 0`<br>`am broadcast -a com.android.systemui.demo -e command exit` |
...
| 获取指定通知的详细信息<br><br> `<notification-key>`格式：`0|tv.danmaku.bili|-2164515038|null|10250`<br>部分安卓版本只需要通知id | `cmd notification get  <notification-key>` |
